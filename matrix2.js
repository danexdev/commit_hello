...
function generateCommitDates(year, text, fillBackground) {
  ...
  const totalColumns = Math.ceil((lastSaturday - firstSunday + 1) / (7 * 24 * 60 * 60 * 1000));
  const rows = 7; // Sunday-Saturday
  const maxLength = Math.floor(totalColumns / 6); // Fit 6 columns per character
  const truncatedText = text.slice(0, maxLength);
  console.log("totalDaysInYear", {totalColumns, rows, maxLength, truncatedText});
...
//     date.setDate(firstSunday.getDate() + rowIndex * 7 + colIndex);
//     return date;
//   })
// );
const matrix = Array.from({ length: rows }, (_, rowIndex) =>
...
const commitDates = []
  ...
  truncatedText.split("").forEach((char, charIndex) => {
    ...
    // console.log("charMap", {charMap,char, charIndex, charStartCol});
